...
class CaptchaEngine:
    def __init__(self, width, height):
        self.width = width
        self.height = height
        self.configPath = resource_path('data.cfg')
        self.weightPath = resource_path('data.weights')
        self.labelPath = resource_path('data.nms')
        self.LABELS_PLATE = open(self.labelPath).read().strip().split('\n')

    def build_model(self):
        net = cv2.dnn_DetectionModel(self.configPath, self.weightPath)
        net.setPreferableBackend(cv2.dnn.DNN_BACKEND_OPENCV)
        net.setPreferableTarget(cv2.dnn.DNN_TARGET_CPU)
        net.setInputSize(self.width, self.height)
        net.setInputScale(0.00392156862745098)
        net.setInputSwapRB(True)
        return net

    def solve_cv2(self, image):
        def get_key_x(item): return item[1]

        net = self.build_model()
        H, W, _ = image.shape
        classes, confidences, boxes = net.detect(image, confThreshold=0.5, nmsThreshold=0.3)
        info = []
        if len(classes) > 0:
